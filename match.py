    ...
    with game_engines(config, active_engines, reusable_engines) as worker:
        ...
        while len(moves) < config.max_plies:
            ...
            if search.bestmove is None:
                ...
                # 1. Ask the other engine to explicitly confirm the game result
                result = confirm_no_legal_moves(
                    other_engine, moves, task.start.fen, search, team, config.timeout
                )
                
                # 2. Fallback: If unconfirmed, trust the previous move ONLY if it announced mate-in-1
                if result is None and searches:
                    prev = searches[-1]
                    prev_score = prev.get("score")
                    if (
                        isinstance(prev_score, dict)
                        and prev_score.get("type") == "mate"
                        and str(prev_score.get("value")) == "1"
                        and prev.get("team") in {"ry", "bg"}
                    ):
                        result = f"{prev['team']}_win"

                termination = "no_legal_moves_result" if result else "no_legal_moves"
                return finish_game(
                    record, result or "draw", termination, moves, clocks, task.engine1_team
                )
